...
int main(int argc, char *argv[])
{
	...
	const char *audioTarget = "plughw:0,0";
	int port = 2306;
	...
	while(1) {
		...
		while(select((int)server+1, &rfds, 0, 0, &timeout)) {	//recupere les donn�es de l'autre programme (waveSend) par le reseau
...
static int configureServer(int port) {
	...
	if(bind(server, (struct sockaddr*)&sin, sizeof(sin)) == -1) {
		logMessage("Erreur: port deja utilis�, choisissez un autre\n");
